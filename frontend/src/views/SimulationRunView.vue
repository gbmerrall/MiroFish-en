...
// Get maxRounds directly from query params during initialization to ensure the child component can get the value immediately
...
const handleGoBack = async () => {
  ...
  try {
    // First, try to gracefully shut down the simulation environment
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Failed to close simulation environment, attempting to force stop...`)
        try {
...
const handleNextStep = () => {
  // The Step3Simulation component will directly handle report generation and routing
...
onMounted(() => {
  ...
  // Record maxRounds configuration (value is already obtained from query params during initialization)
  if (maxRounds.value) {
