    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Starting...' : 'Start Generating Results Report' }} 
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Check if simulation is completed (judged by runner_status or platform completion status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: if backend hasn't updated runner_status yet, but platform has reported completion
      // Judged by checking twitter_completed and reddit_completed status
...
const checkPlatformsCompleted = (data) => {
  ...
  // Judged by actions_count to determine if platform is enabled (if count > 0 or running was true)
