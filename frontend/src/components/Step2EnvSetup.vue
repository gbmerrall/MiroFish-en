    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combining the context, automatically use tools to sort out entities and relationships from the knowledge graph, initialize simulation individuals, and endow them with unique behaviors and memories based on real-world seeds.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM intelligently sets parameters such as the world's time flow rate, recommendation algorithms, active time periods for each individual, speech frequency, and event triggers based on the simulation requirements and real-world seeds.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation rounds configuration - only shown after the configuration is generated and the number of rounds is calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish automatically plans to deduce reality for <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, with each round representing <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of elapsed time in reality.</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If the agent scale is 100: the estimated time is about {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        If the agent scale is 100: the estimated time is {{ Math.round(autoGeneratedRounds * 0.6) }} minutes
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">If this is your first run, it is highly recommended to switch to 'Custom Mode' to reduce the number of simulation rounds, in order to quickly preview the effect and reduce the risk of errors ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Summary of experiences and stylistic preferences</span>
              </div>
              <div class="dimension-card">
...
// Log deduplication: records key information from the last output
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Simulation round configuration
const useCustomRounds = ref(false) // Default to automatic round configuration
const customMaxRounds = ref(40)   // Default recommendation is 40 rounds
...
// Calculate automatically generated rounds from configuration (not using hardcoded defaults)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Return null if configuration is not generated
  }
  ...
  // Ensure maximum rounds are not less than 40 (recommended value) to avoid abnormal slider range
...
const handleStartSimulation = () => {
  // Construct parameters to pass to parent component
  const params = {}
...
const startPrepareSimulation = async () => {
  ...
  // Mark the first step as complete, start the second step
  phase.value = 1
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Output configuration generation stage log (avoid duplication)
...
const loadPreparedData = async () => {
  ...
  // Get config (use real-time interface)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Configuration generating, starting polling to wait...')
        startConfigPolling()
      }
    }
  } catch (err) {
