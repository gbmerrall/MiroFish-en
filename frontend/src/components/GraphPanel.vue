...
const wasSimulating = ref(false) // Track if it was previously simulating
...
// Watch for changes in isSimulating to detect when simulation ends
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Show finished hint when transitioning from simulating to not simulating
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle the expanded/collapsed state of a self-loop item
...
const renderGraph = () => {
  ...
  // Process edge data, calculate number of edges and index between the same pair of nodes
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loop edges grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count the number of edges between each pair of nodes, collect self-loop edges
  tempEdges.forEach(e => {
  ...
  // Record which edge of each pair is currently being processed
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Calculate curvature: spread out multiple edges, straight for single edge
    let curvature = 0
    if (totalCount > 1) {
      ...
      // This ensures all edges are distributed under the same reference frame and won't overlap due to direction differences
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - dynamically adjust node distance based on number of edges
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity towards center to gather independent node groups to the central area
  ...
  const getLinkPath = (d) => {
    ...
    // More edges mean a larger offset ratio relative to distance
  ...
  // Store references to control visibility externally
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only, don't restart simulation (distinguish click and drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Check if drag has truly started (moved past threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart simulation only on first detection of true drag
          d._isDragging = true
