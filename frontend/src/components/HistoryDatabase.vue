<template>
  ...
  >
    ...
    <!-- Card container (only displayed when there are projects) -->
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card title (use the first 20 characters of the simulation requirement as the title) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Start Simulation" and Step 5 "Deep Interaction" must be started during runtime and do not support history playback.</span>
            </div>
          </div>
        </div>
      </Transition>
...
// Card layout configuration - adjusted for a wider ratio
...
const containerStyle = computed(() => {
  ...
  // Calculate the actual required height: rows * card height + (rows - 1) * gap + a small bottom margin
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Expand downwards, increasing the distance from the title
    ...
    const x = offset * 35
    // Adjust the starting position to be close to the title but maintain a proper distance
...
// Format date (show only the date part)
...
// Generate title from simulation requirement (take the first 20 characters)
...
// Format simulation_id display (take the first 6 characters)
...
// Truncate filename (keep the extension)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the target state to be executed (always record the latest target state regardless of whether it is animating)
        pendingState = shouldExpand
        
        // Clear the previous debounce timer (new scroll intent will override the old one)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // If it is animating, only record the state and process it after the animation ends
        if (isAnimating) return
        
        // If the target state is the same as the current state, no need to process
        ...
        // The delay for expanding is shorter (50ms), and the delay for collapsing is longer (200ms) to increase stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if the pending state still needs to be executed (may have been overridden by subsequent scrolling)
          ...
          // Release the lock after the animation is complete, and check if there are any pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // After the animation ends, check if there is a new pending state
    ...
    },
    {
      ...
      // Adjust rootMargin, the bottom of the viewport shrinks upwards, requiring more scrolling to trigger expansion
...
// Watch for route changes, reload data when returning to the homepage
...
.grid-pattern {
  ...
  /* Position from top-left, expand at the bottom on height change without affecting grid positions */
...
.cards-container {
  ...
  /* min-height is calculated by JS based on card count */
}
...
/* Low-saturation color palette - Morandi colors */
