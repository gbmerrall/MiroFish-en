...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search interface, so it needs to search edges and nodes separately and then merge the results.
        Use parallel requests to search simultaneously for improved efficiency.
...
".join(related_info))
        ...
        if zep_results.get("facts"):
            ...
            if new_facts:
                context_parts.append("### Zep Retrieved Fact Information
" + "
...
".join(context_parts)
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Determines if it is an individual entity type"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determines if it is a group/institution entity type"""
    ...
    def _generate_profile_with_llm(
        ...
        - Group/institution entity: generates representative account settings
        """
        ...
        # Attempt to generate multiple times until successful or maximum retries reached
        max_attempts = 3
        last_error = None
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if last character after last quote is not a comma or closing bracket, string might be truncated
...
', ' ').replace('', ' ')
            ...
            try:
                ...
                # 5. If still fails, try a more aggressive fix
                try:
        ...
        # 6. If completely failed, return base structure
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        if is_individual:
            return "You are an expert in social media user persona generation. Generate detailed, realistic personas for public opinion simulation, maximally restoring existing real-world conditions. Must return valid JSON format, all string values must not contain unescaped newlines."
        else:
            return "You are an expert in social media institutional account persona generation. Generate detailed, realistic account settings for public opinion simulation, maximally restoring existing real-world conditions. Must return valid JSON format, all string values must not contain unescaped newlines."
    ...
    def _build_individual_persona_prompt(
        ...
        """
        Builds a detailed persona prompt for an individual entity.
        """
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Stance/viewpoint (attitude towards topics, content that might provoke/move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of the persona, introduce the individual's connection to the event, and their existing actions and reactions in the event)
3. age: Age (must be an integer)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """
        Builds a detailed persona prompt for a group/institution entity.
        """
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate a detailed social media account setting for the institution/group entity, maximally restoring existing real-world conditions.
...
2. persona: Detailed account setting description (2000 characters of plain text), must include:
   - Institution basic information (official name, institution nature, establishment background, main functions)
   ...
   - Content release characteristics (content type, release frequency, active time periods)
   - Stance/attitude (official stance on core topics, how to handle controversies)
   - Special notes (profile of the represented group, operating habits)
   - Institutional memory (important part of the institution persona, introduce the institution's connection to the event, and its existing actions and reactions in the event)
3. age: Fixed to 30 (virtual age of institutional account)
4. gender: Fixed to "other" (institutional account uses other for non-individual)
...
- Institutional account statements must conform to its identity positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        """Generates a basic persona based on rules"""
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "age": 30,  # Virtual age of institution
    ...
    def generate_profiles_from_entities(
        ...
        completed_count = [0]  # Use a list to modify in closure
        lock = Lock()
...
".join(output_lines)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter profiles in CSV format (meets OASIS official requirements).
        ...
        - description: External display, public bio visible to other users
        """
        import csv
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                "country": profile.country if profile.country else "USA",
            }
