...
class EntityNode:
    ...
    def get_entity_type(self) -> Optional[str]:
        """Gets entity type (excluding default Entity label)"""
...
class ZepEntityReader:
    ...
    Main Features:
    1. Reads all nodes from Zep graph.
    2. Filters nodes matching predefined entity types (Labels not just Entity).
    3. Fetches related edges and associated node info for each entity.
    """
    ...
    def _call_with_retry(
        ...
        Args:
            ...
            max_retries: Max retry attempts (default 3, meaning at most 3 attempts)
    ...
    def get_all_nodes(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Fetched total of {len(nodes_data)} nodes")
        return nodes_data

    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Gets all edges in the graph (paginated)
        ...
        logger.info(f"Fetched total of {len(edges_data)} edges")
        return edges_data
    
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Gets all related edges for a specified node (with retry mechanism)
    ...
    def filter_defined_entities(
        ...
        """
        Filters nodes matching predefined entity types
        
        Filtering logic:
        - If node Labels only contains "Entity", skip it as it doesn't match predefined types.
        - If node Labels contain labels other than "Entity" and "Node", retain it as matching predefined types.
        
        Args:
            graph_id: Graph ID
            defined_entity_types: List of predefined entity types (optional, if provided, only keep these)
        ...
        # Fetch all edges (used for subsequent correlation search)
    ...
    def get_entity_with_context(
        ...
        """
        Gets a single entity with its full context (edges and associated nodes, with retry mechanism)
        ...
        try:
            ...
            node = self._call_with_retry(
                ...
                operation_name=f"Fetching node detail(uuid={entity_uuid[:8]}...)"
            )
            ...
            # Get all nodes for correlation lookup
            all_nodes = self.get_all_nodes(graph_id)
            node_map = {n["uuid"]: n for n in all_nodes}
