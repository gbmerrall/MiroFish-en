...
def build_graph():
    ...
    try:
        ...
        if not text:
            return jsonify(
                {"success": False, "error": "No extracted text content found"}
            ), 400
        ...
        if not ontology:
            return jsonify(
                {"success": False, "error": "No ontology definition found"}
            ), 400
        ...
        def build_task():
            ...
            try:
                ...
                # Create graph
                task_manager.update_task(
                    task_id, message="Creating Graphiti graph...", progress=10
                )
